 ...
 */
public class MeanShift implements ExtendedPlugInFilter, DialogListener{
...
//        showProgress( 1.0);
    ...
    public static BufferedImage filterRGBImage(ImagePlus ip)
    {
        int rad = 30; //20
        int rad2 = rad * rad;
        float radCol = 40.0f; //30.0f
        float radCol2 = radCol * radCol;
        int width = ip.getWidth();
        int height = ip.getHeight();
        BufferedImage result = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
        int[] pixels = ip.getBufferedImage().getRGB(0, 0, width, height, null, 0, width);
        ...
        for (int y=0; y<height; y++) {
            if (y % 10 == 0) {
                System.out.print("\rMean shift @ " + (y+10) + " / " + height + ".");
